...
const wholeNumber = (value) => Math.max(0, Math.min(9999, Math.round(Number(value) || 0)));

/** Classement publié par l'écran de soirée : des noms et trois compteurs, rien d'autre. */
function cleanBoard(raw) {
  const board = {};
  for (const [name, stats] of Object.entries(raw && typeof raw === "object" ? raw : {}).slice(0, MAX_PLAYERS)) {
    const player = cleanText(name, 40);
    if (!player) continue;
    board[player] = {
      points: wholeNumber(stats?.points),
      right: wholeNumber(stats?.right),
      votes: wholeNumber(stats?.votes),
    };
  }
  return board;
}
...
export function validateRound({ code, round }) {
  ...
  return {
    ...
    value: {
      ...
      players,
      board: cleanBoard(source.board),
      bonus: source.bonus !== false,
      revealed,
