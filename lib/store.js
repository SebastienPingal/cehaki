...
/**
 * Paris d'un tour, avec leur contenu. Réservé aux tours révélés : avant la
 * révélation, seuls les noms sortent d'ici (cf. `listVoters`).
 */
export async function listVotes(code, roundId) {
  ...
  return entries
    .map(([voter, value]) => {
      let vote = {};
      try {
        vote = JSON.parse(value) || {};
      } catch { /* valeur illisible : le nom suffit */ }
      return { name: voter, guess: String(vote.guess || ""), at: vote.at || 0 };
    })
    .sort((a, b) => a.at - b.at);
}
 ...
 * jamais ce qu'il a voté tant que le morceau n'est pas révélé.
 */
export async function listVoters(code, roundId) {
  return (await listVotes(code, roundId)).map(({ name, at }) => ({ name, at }));
}
